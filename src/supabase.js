...
export async function fetchFlash(){return fetch(`${url}/rest/v1/flash?select=*&order=sort_order.desc,created_at.desc`,{headers:headers(getSession()?.access_token)}).then(json)}
...
export async function removeImage(path){return fetch(`${url}/storage/v1/object/flash-images/${path}`,{method:'DELETE',headers:headers(getSession()?.access_token)}).then(json)}
...
export async function fetchTattoos(){return fetch(`${url}/rest/v1/tattoo_gallery?select=*&order=sort_order.desc,created_at.desc`,{headers:headers(getSession()?.access_token)}).then(json)}
...
export async function removeTattooImage(path){return fetch(`${url}/storage/v1/object/tattoo-images/${path}`,{method:'DELETE',headers:headers(getSession()?.access_token)}).then(json)}
